...
export default function AdminCustomersPage() {
  ...
  const confirmLock = async () => {
    ...
    setLocking(true);
    try {
      await adminApi.lockUser(lockTarget.id, lockReason || "Vi phạm quy định");
      toast.success(`Đã khóa tài khoản ${lockTarget.email}`);
  ...
  const unlockCustomer = async (customer: Customer) => {
    try {
      await adminApi.unlockUser(customer.id);
  ...
  const columns: Column<Customer>[] = [
    {
      ...
      label: "Tên",
      render: (item) => (
        <button onClick={() => viewDetail(item.id)} className="font-medium text-primary-800 hover:underline">
          {item.name}
        </button>
      ),
    },
    ...
    { key: "createdAt", label: "Ngày tham gia", render: (item) => formatDate(item.createdAt) },
    {
      ...
      render: (item) => (
        <div className="flex justify-end gap-1">
          <button onClick={() => openEdit(item)} className="rounded p-1.5 text-gray-500 hover:bg-gray-100 hover:text-gray-900" title="Sửa">
            <Edit className="h-4 w-4" />
          </button>
          {item.isLocked ? (
            <button onClick={() => unlockCustomer(item)} className="rounded p-1.5 text-gray-500 hover:bg-emerald-50 hover:text-emerald-700" title="Mở khóa">
              <Unlock className="h-4 w-4" />
            </button>
          ) : (
            <button onClick={() => openLockModal(item)} className="rounded p-1.5 text-gray-500 hover:bg-red-50 hover:text-red-700" title="Khóa">
              <Lock className="h-4 w-4" />
            </button>
          )}
          <button onClick={() => deleteCustomer(item)} className="rounded p-1.5 text-gray-500 hover:bg-red-50 hover:text-red-700" title="Xóa mềm">
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-wrap items-center justify-between gap-3">
        ...
        <button onClick={openCreate} className="btn-primary inline-flex items-center gap-2 px-4 py-2">
      ...
      <div className="flex flex-wrap gap-2">
        <form onSubmit={handleSearch} className="flex min-w-80 max-w-md flex-1 gap-2">
          ...
          <button type="submit" className="btn-primary px-4 py-2">Tìm</button>
        </form>
        ...
        <div className="fixed inset-0 z-[110] flex items-center justify-center">
          ...
          <form onSubmit={saveCustomer} className="relative mx-4 w-full max-w-lg rounded-lg bg-white shadow-xl">
            <div className="flex items-center justify-between border-b border-gray-200 px-6 py-4">
              ...
              <button type="button" onClick={() => setFormOpen(false)} className="rounded p-1 hover:bg-gray-100"><X className="h-5 w-5" /></button>
            </div>
            ...
            <div className="flex justify-end gap-2 border-t border-gray-200 px-6 py-4">
              <button type="button" onClick={() => setFormOpen(false)} className="rounded-md border border-gray-200 px-4 py-2 text-sm">Hủy</button>
              <button type="submit" disabled={saving} className="btn-primary px-5">{saving ? "Đang lưu..." : "Lưu"}</button>
            </div>
          </form>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-[120] flex items-center justify-center">
          ...
          <div className="relative mx-4 w-full max-w-md rounded-xl bg-white shadow-xl">
                ...
                <div className="mt-2 flex flex-wrap gap-1.5">
                  {["Vi phạm quy định", "Spam/lừa đảo", "Yêu cầu từ khách hàng", "Tài khoản trùng lặp"].map((reason) => (
                    <button
                      ...
                      className="rounded-full border border-gray-200 px-3 py-1 text-xs text-gray-600 transition-colors hover:border-red-200 hover:bg-red-50 hover:text-red-700"
                    >
                      {reason}
                    </button>
                  ))}
            ...
            <div className="flex justify-end gap-2 border-t border-gray-100 px-6 py-4">
              <button type="button" onClick={() => setLockModalOpen(false)} className="rounded-lg border border-gray-200 px-4 py-2 text-sm font-medium text-gray-600 hover:bg-gray-50">
                Hủy
              </button>
              <button
                type="button"
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center">
          ...
          <div className="relative mx-4 max-h-[80vh] w-full max-w-2xl overflow-y-auto rounded-lg bg-white shadow-xl">
            <div className="sticky top-0 flex items-center justify-between border-b border-gray-200 bg-white px-6 py-4">
              ...
              <button onClick={() => setSelectedCustomer(null)} className="rounded p-1 hover:bg-gray-100"><X className="h-5 w-5" /></button>
            </div>
